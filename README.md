...
VR 전용 미니멀 데스크핏을 지향합니다. VR 헤드셋을 쓰고 손을 뻗은 곳에 스위치가 있었으면 하는 생각을 1년 이상 참다가 결국 만들었습니다 ㅠㅠ 실기 1:1 스케일로 제작했습니다. 사용하지 않을 때 쉽게 치울 수 있는 이동식/수납식 구조이며, 인게임 디스플레이로 충분한 계기류(DED, RWR, 엔진 게이지, HSI 등)는 제작하지 않고 스위치와 LED 위주로 구성했습니다.

## Features

- **Left Aux Panels 위주로 제작** — Gear, CMDS, TWA, Alt Gear로 구성된 Left Aux Panels 위주로 제작하고, I2C로 MISC 패널을 보조로 제작.
...
- **MCP23017 I2C 확장** — I2C(MCP23017)를 이용하여 새로운 패널을 여러 개 추가 가능. MISC 패널은 현재 MCP23017으로 제작하여 Left Aux Panels의 slave 장치로 동작하며, 랜선 8중 4핀으로 연결 가능하므로 확장성이 뛰어남. 기존 쉬프트레지스터 방식은 칩 별로 In/Out이 정해져 있지만 I2C는 칩의 모든 핀을 In/Out으롤 설정 가능한 장점이 있음. 또한, 데이지체인 형태로 계속 slave 장치를 추가 가능하므로 향후 다른 패널들 (Snsr, HUD 등) 대응도 마음만 먹으면 가능
...
- **전원**: Teensy 4.1의 Vin 핀(USB 3.0 5V 출력)에서 공급받아 12V로 boost하여 사용
- **LED**: 3V / 20mA 녹색 고휘도 3mm LED
- **회로 구성**: LED 3개 + 150Ω 저항 1개로 구성된 직렬 스트링을 최대 15개까지 병렬로 분산 배치 (20mA × 15 = 300mA @ 12V)
- **배치 방법**: PC Flights 패널은 맨 뒤 패널이 투명하므로, 뒤쪽에 적당한 거리에서 고휘도 LED를 자유롭게 배치하면 됨
...
Windows의 기본 Joystick은 32버튼까지만 지원하지만, Teensyduino의 Extreme Joystick 모드는 128버튼을 지원합니다. 현재 50버튼을 사용하며 I2C 패널 확장 시 더 늘어나므로 이 모드가 필요합니다. Teensy 빌드 전 `%LOCALAPPDATA%\Arduino15\packages\teensy\hardware\avr\<version>\cores\teensy4\usb_desc.h`에서 `JOYSTICK_SIZE`를 `64`로 변경:
...
Teensy의 다이렉트 핀은 모두 사용 중이므로, 추가 스위치/LED는 MCP23017 I2C 확장으로만 추가할 수 있습니다. MCP23017 하나당 16핀(GPA0–7, GPB0–7)을 제공하며, I2C 주소를 달리하여 최대 8개(0x20–0x27)까지 데이지체인 연결이 가능합니다.
